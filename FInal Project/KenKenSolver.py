...
class KenKenSolverGUI:
    ...
    def generate_random_cages(self, grid_size):
        ...
        for _ in range(grid_size):  # Generate a number of cages equal to grid size
            ...
            while len(cage_cells) < 2:  # Minimum of 2 cells per cage
                ...
                if (row, col) not in visited and (row, col) not in cage_cells:
                    cage_cells.append((row, col))
                    visited.add((row, col))

            # Randomly determine the operation and target for the cage
            target = random.randint(1, grid_size * 2)  # Simple target generation
    ...
    def start_solving(self):
        ...
        cages = self.random_cages  # Use generated random cages
        if algorithm == "Backtracking":
            solver = KenKenSolver(selected_size, cages)
            start_time = time.time()
            if solver.solve():
                elapsed_time = time.time() - start_time
                self.display_solution(solver.board)
                print(f"Backtracking solution found in {elapsed_time:.2f} seconds")
            else:
                messagebox.showerror("Error", "No Solution Found")
        elif algorithm == "Genetic Algorithm":
            solver = GeneticKenKenSolver(selected_size, cages)
            start_time = time.time()
            if solver.solve():
                elapsed_time = time.time() - start_time
                self.display_solution(solver.board)
                print(f"Genetic solution found in {elapsed_time:.2f} seconds")
            else:
                messagebox.showerror("Error", "No Solution Found")
